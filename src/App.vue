  ...
  <div class="min-h-screen flex flex-col" style="background-color:#0f172a; font-family:'JetBrains Mono',monospace;">
    ...
    >
      ...
      <div class="flex items-center gap-4">
        ...
        >
          THE LEGACY<br class="sm:hidden" />
          <span style="color:#ff00ff;"> REVIVER</span>
        </h1>
      </div>
      ...
      <div class="flex items-center gap-6">
        <!-- Mission badge -->
        <div class="hidden sm:flex items-center gap-2 border-2 px-3 py-1"
          :style="`border-color:${missionStatus==='COMPLETE'?'#39ff14':'#ff5555'}; box-shadow:3px 3px 0 ${missionStatus==='COMPLETE'?'#39ff14':'#ff5555'};`"
        >
          <span class="w-2 h-2 rounded-none animate-pulse inline-block"
            :style="`background:${missionStatus==='COMPLETE'?'#39ff14':'#ff5555'};`"
          ></span>
          <span class="text-xs" style="font-family:'VT323',monospace; font-size:1.1rem; letter-spacing:0.1em;"
            :style="`color:${missionStatus==='COMPLETE'?'#39ff14':'#ff5555'};`"
          >
        ...
        <div
          ...
          style="border-color:#00ffff; box-shadow:3px 3px 0 #00ffff; color:#00ffff; font-family:'VT323',monospace; font-size:1.4rem; letter-spacing:0.15em;"
        >
          {{ currentTime }}
        </div>
      </div>
    ...
    <!-- ══════════════════════════════════════════════
         MAIN CONTENT
    ══════════════════════════════════════════════ -->
    <main class="flex-1 flex flex-col gap-0 p-6" style="gap:1.5rem;">

      <!-- Mission Banner -->
      <div
        class="border-2 px-5 py-3 flex items-center justify-between flex-wrap gap-3"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
        ...
        <section id="code-editor-panel" class="flex flex-col border-2" style="border-color:#39ff14; box-shadow:6px 6px 0 #39ff14; min-height:480px;">
          ...
          <!-- Editor Title Bar (retro OS window) -->
          <div class="flex items-center justify-between px-4 py-2 border-b-2" style="border-color:#39ff14; background-color:#0d1a0d;">
            <!-- Window control dots (blocky) -->
            <div class="flex items-center gap-2">
              <button
                class="w-4 h-4 border cursor-pointer transition-colors"
                style="border-color:#ff5555; background:#ff5555;"
                title="Close"
                @click="() => {}"
              ></button>
              <button
                class="w-4 h-4 border cursor-pointer transition-colors"
                style="border-color:#ffe600; background:#ffe600;"
                title="Minimize"
                @click="() => {}"
              ></button>
              <button
                class="w-4 h-4 border cursor-pointer transition-colors"
                style="border-color:#39ff14; background:#39ff14;"
                title="Maximize"
                @click="() => {}"
              ></button>
            </div>
            <!-- File label -->
            <span class="text-xs" style="color:#39ff14; font-family:'VT323',monospace; font-size:1.05rem; letter-spacing:0.1em;">
              legacy_sort.js — THE WORKSPACE
            </span>
            <!-- Line count -->
            <span class="text-xs" style="color:#475569; font-family:'JetBrains Mono',monospace;">
              {{ codeValue.split('\n').length }} LINES
            </span>
          </div>
        ...
        <section id="terminal-panel" class="flex flex-col border-2" style="border-color:#ff00ff; box-shadow:6px 6px 0 #ff00ff; min-height:480px;">
          ...
          >
            <template v-for="(log, i) in terminalLogs" :key="i">
              <p
                v-if="log.type === 'divider'"
                style="color:#1e3a2e; user-select:none;"
              >{{ log.text }}</p>
      ...
      <!-- ══════════════════════════════════════════════
           SUBMIT BUTTON (Neo-Brutalist CTA)
      ══════════════════════════════════════════════ -->
      <div class="flex justify-center pt-2 pb-4">
        <button
          ...
          :style="`
            border-color: ${submitDone ? '#39ff14' : '#ff00ff'};
            background: ${isSubmitting ? '#1a001a' : submitDone ? '#001a00' : '#1a001a'};
            box-shadow: 8px 8px 0 ${submitDone ? '#39ff14' : '#ff00ff'};
        ...
        >
          <span
            class="btn-label block text-center transition-colors"
