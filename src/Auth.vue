...
async function submitAuth(e) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      setTimeout(() => {
        // Mock: treat "fail@test.com" as a server error
...
<template>
  <!-- Root: full-viewport, dark charcoal, subtle grid overlay -->
  <div
  ...
  >
    ...
    >
      ...
      <!-- Card Header -->
      <div
        class="border-b-4 px-6 py-5"
        style="border-color: #39ff14; background-color: #0a180a;"
      >
        <!-- Logo row -->
        <div class="flex items-center gap-3 mb-4">
          <div
            class="w-9 h-9 border-2 flex items-center justify-center text-xs font-bold flex-shrink-0"
          ...
          <span
            ...
            style="font-family:'Press Start 2P',monospace; color:#39ff14; letter-spacing:0.04em;"
          >THE LEGACY<span style="color:#ff00ff;"> REVIVER</span></span>
        </div>
      ...
      >
        ...
        <div class="flex flex-col gap-1">
          ...
          <input
            ...
            v-model="email"
            type="email"
            placeholder="operative@legacynet.io"
            autocomplete="email"
            :disabled="isLoading"
            class="w-full px-4 py-3 border-2 outline-none text-sm transition-colors disabled:opacity-50"
            style="
              background:#121212;
              color:#f8fafc;
              border-color:#334155;
        ...
        <!-- Username (Register only) — v-show keeps layout stable during transition -->
        <Transition name="slide">
          <div v-if="!isLoginView" class="flex flex-col gap-1">
            <label
              ...
              class="text-xs"
              style="color:#94a3b8; letter-spacing:0.1em;"
            >OPERATIVE_HANDLE</label>
            <input
              id="auth-username"
              v-model="username"
              type="text"
              placeholder="e.g. ghost_r3v1v3r"
        ...
        <Transition name="fade">
          <div
            v-if="errorMessage"
            class="border-2 px-4 py-3 text-xs"
        ...
        <button
          ...
          type="submit"
          :disabled="isLoading"
          class="w-full border-4 px-6 py-4 text-sm font-bold transition-all duration-100 cursor-pointer disabled:cursor-not-allowed disabled:opacity-60 mt-1"
          style="
      ...
      <!-- Toggle Mode -->
      <div
        class="border-t-2 px-6 py-4 text-center"
        style="border-color:#1e2d1e; background:#0a0a0a;"
      >
        <button
          id="auth-toggle-btn"
          type="button"
          @click="toggleMode"
          :disabled="isLoading"
          class="text-xs transition-colors disabled:opacity-50 cursor-pointer underline underline-offset-4"
...
<style scoped>
/* Slide transition for username field */
.slide-enter-active,
.slide-leave-active {
